...
class TrieTree
{
     ...
     * @throws \Exception
     */
    public function initDictionary($path)
    {
        if (file_exists($path)) {
            $file = fopen($path, "r");
            $i = 0;
            ...
            while (!feof($file)) {
                ...
                $this->append($list[0], ['level' => trim($list[1])]);
            }
            fclose($file);
        } else {
            $message = "dictionary path didn't exist !";
            $code = '404';
    ...
    public function search($search)
    {
            ...
            if (isset($tree[$search_keys[$i]])) {
                ...
                if ($node['end']) {
                    ...
                    if (isset($hit_arr[$key])) {
                        ...
                        //判断等级
                        if (isset($node['data']['level']) && $node['data']['level'] == 1) {
                            //替换
                            $len = mb_strlen($node['full']);
                            $replaceStr = str_repeat("*", $len);
                            $search = $this->mb_substr_replace($search, $replaceStr, $current_index, $len);
                        } elseif (isset($node['data']['level']) && $node['data']['level'] == 2) {
                            //禁用词汇
                            $search = "";
                            $hit_arr = [];
                            $hit_arr['word'] = $node['full'];
                            $hit_arr['error']['code'] = '403';
                            $hit_arr['error']['message'] = "禁用词汇";
                            break;
                        }
                    }
                    if (empty($node['child'])) {
        ...
        $result['original'] = $search;
        $result['hit'] = $hit_arr;
        return $result;
    }

    private function mb_substr_replace($search, $replaceStr, $current_index, $len)
    {
        if ($current_index) {
            $front = mb_substr($search, 0, $current_index);
        } else {
            $front = "";
        }
        $behind = mb_substr($search, $current_index + $len, -1);
        return $front . $replaceStr . $behind;
    }
